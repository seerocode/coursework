...
#log price(
ln_regres_pxy <- glm(formula = logmove ~ price.x + price.y * feat * brand * log(price.x)*ETHNIC * log(price.x)*HVAL150 + log(price.x)*WORKWOM * log(price.x)*HVAL150 *log(price.x)*EDUC, data = oj_with_lagged_prices)
summary(ln_regres)
